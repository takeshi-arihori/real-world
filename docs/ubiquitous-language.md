...
| BrowserSession | First-party frontend を認証する BFF の server-side session。opaque cookie で識別し JWT を browser に公開しない | BFF adapter, AuthProvider refresh | BrowserToken |
...
- First-party frontend は同一 origin の BFF が管理する BrowserSession を利用し、JWT、session identifier、refresh token を JavaScript から読める storage / state に保存しない。
